...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'MigrateCitrixUPMtoFSLogix.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'MigrateCitrixUPMtoFSLogix.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'MigrateCitrixUPMtoFSLogix.Client.ProtectedToken'
