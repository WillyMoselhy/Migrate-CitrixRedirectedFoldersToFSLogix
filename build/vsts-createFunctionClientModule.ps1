...
$parentModule = 'MigrateCitrixUPMtoFSLogix'
if (-not $ModuleName) { $ModuleName = 'MigrateCitrixUPMtoFSLogix.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\MigrateCitrixUPMtoFSLogix'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\MigrateCitrixUPMtoFSLogix.psd1"
$prereqHash = @{
